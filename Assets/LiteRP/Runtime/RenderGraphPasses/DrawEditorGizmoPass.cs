...
namespace LiteRP
{
    ...
    { 
        ...
        {
            internal RendererListHandle editorGizmoRenderListHandle;
        }
        ...
        private void AddDrawEditorGizmoPass(RenderGraph renderGraph, CameraData cameraData, GizmoSubset gizmoSubset)
        {
                       ...
                       s_DrawEditorGizmoProfilingSampler))
            {
                ...
                passData.editorGizmoRenderListHandle = renderGraph.CreateGizmoRendererList(cameraData.camera, gizmoSubset);
                builder.UseRendererList(passData.editorGizmoRenderListHandle);
                builder.AllowPassCulling(false);
                
                builder.SetRenderFunc((DrawEditorGizmoPassData data, RasterGraphContext context) =>
                {
                    context.cmd.DrawRendererList(data.editorGizmoRenderListHandle);
                });
            }
        }
    }
